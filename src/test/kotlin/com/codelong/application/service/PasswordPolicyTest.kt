...
class PasswordPolicyTest {
    ...
    fun `rejeita senha abaixo do minimo`(length: Int) {
        val error = assertThrows<InvalidInputException> { policy.requireStrong("a".repeat(length)) }
    ...
    fun `rejeita senha vazia`() {
        val error = assertThrows<InvalidInputException> { policy.requireStrong("") }
    ...
    fun `politica customizada usa os limites informados`() {
        ...
        assertThrows<InvalidInputException> { custom.requireStrong("ab") }
        assertThrows<InvalidInputException> { custom.requireStrong("abcdef") }
    }
    ...
    fun `politica customizada reporta os proprios limites na mensagem`() {
        ...
        val tooShort = assertThrows<InvalidInputException> { custom.requireStrong("curta") }
        val tooLong = assertThrows<InvalidInputException> { custom.requireStrong("x".repeat(21)) }
