...
- **Modos**: `CLASSIC` (todas as perguntas, dificuldade crescente), `GENOCIDA` (todas, ordem aleatoria, morte subita), `APRENDIZADO` (perguntas de um **tema/categoria**, dificuldade crescente, morte subita) e `GUBEE` (perguntas **exclusivas** da categoria GUBEE, dificuldade crescente, sem morte subita). Nos modos de morte subita a resposta errada encerra a partida como `DEFEATED`.
- **GUBEE**: as perguntas da categoria `GUBEE` sao exclusivas do modo GUBEE — ficam fora do Classico, Genocida e dos temas do Aprendizado. Como ainda nao ha perguntas GUBEE, iniciar o modo retorna `409 NO_ACTIVE_QUESTIONS`. Ranking proprio (`GET /api/rankings?mode=GUBEE`), minimo 10 respostas.
- **Ranking**: entram **todas as partidas** `COMPLETED`, `ABANDONED` e `DEFEATED` com **no minimo 10 respostas (CLASSIC/GUBEE) ou 5 (GENOCIDA/APRENDIZADO)**. O ranking e **separado por modo** (`GET /api/rankings?mode=CLASSIC|GENOCIDA|APRENDIZADO|GUBEE`); o Aprendizado ainda exige o tema (`&theme=KOTLIN`). Sem `mode`, o ranking global considera **apenas Classico e Genocida**. Desempate: `score` ↓ → menor tempo → mais acertos → data mais antiga.
